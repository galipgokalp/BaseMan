#!/usr/bin/env node
/*
 Mainnet smoke test for BaseMan dev server
 Checks:
  1) GET / -> 200
  2) GET /.well-known/farcaster.json has eip155:8453
  3) GET /__env.js has NEXT_PUBLIC_REGISTRY_ADDRESS and NEXT_PUBLIC_PAYMASTER_URL=/api/paymaster-proxy
  4) POST /api/score-sign returns JSON with chainId=8453 and contractAddress when chain='base'
  5) POST /api/paymaster-proxy blocks non-allowlisted target (403)
  6) POST /api/paymaster-proxy forwards allowlisted registry+selector on chain 8453 (status != 403)
*/
...
async function main() {
  ...
  // 2) Manifest includes eip155:8453
  try {
    const { status, json } = await getJson('/.well-known/farcaster.json');
    const reqChains = json?.miniapp?.requiredChains || [];
    const ok = status === 200 && Array.isArray(reqChains) && reqChains.includes('eip155:8453');
    log(ok, `manifest requiredChains includes eip155:8453 (status ${status})`);
    if (!ok) fail++;
  } catch (e) {
  ...
  try {
    ...
    const ok = r.status === 200 && j?.chainId === 8453 && /^0x[0-9a-fA-F]{40}$/.test(j?.contractAddress || '');
    log(ok, `score-sign(base) -> status ${r.status}, chainId=${j?.chainId}, contract=${j?.contractAddress || 'n/a'}`);
    if (!ok) fail++;
  } catch (e) {
    log(false, `score-sign error: ${e.message}`); fail++;
  }

  // Resolve registry address from __env.js
  let registry = env.NEXT_PUBLIC_BASE_MAINNET_REGISTRY_ADDRESS || env.NEXT_PUBLIC_REGISTRY_ADDRESS || null;
  if (!registry) {
    log(false, 'could not resolve registryAddress from __env.js');
    fail++;
  }
  ...
  // 6) paymaster-proxy positive (registry + allowlisted selector on chain 8453)
  try {
    ...
    const ok = r.status !== 403; // forwarded to upstream (may be 200/401/etc.)
